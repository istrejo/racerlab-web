...
export class AuthSessionService implements OnDestroy {
  ...
  readonly isAuthenticated = computed(() => this.authTokenService.hasValidAccessToken());
    ...
    const restoration$ = this.refreshAccessToken(
      ...
      catchError((error: unknown) => {
        if (error instanceof AuthRefreshSupersededError) {
          return of(
            this.hasValidAccessToken() ? ('authenticated' as const) : ('anonymous' as const),
          );
        }
  ...
  startSession(response: AuthTokenResponse): void {
  ...
  hasValidAccessToken(): boolean {
    return this.authTokenService.hasValidAccessToken();
  }

  needsRefresh(windowMs?: number): boolean {
    return this.authTokenService.needsRefresh(windowMs);
  }
  ...
  private validReplacementToken(generation: number, staleAccessToken?: string): string | undefined {
    if (generation === this.sessionGeneration) {
      return undefined;
    }
  ...
  private hasSameAccessContext(firstToken: string, secondToken: string): boolean {
    return this.authTokenService.hasSameAccessContext(firstToken, secondToken);
  }
  ...
  private acceptTokenResponse(response: AuthTokenResponse, clearOnFailure = true): void {
    const metadata = this.authTokenService.readAccessTokenMetadata(response?.accessToken);
    ...
    const identityChanged = Boolean(
      metadata.subject &&
      this.authTokenService.getSubject() &&
      metadata.subject !== this.authTokenService.getSubject(),
    );
    ...
    this.authTokenService.setAccessToken(response.accessToken, metadata);
  ...
  private clearSession(broadcast: boolean, broadcastReason: AuthLogoutReason = 'logout'): void {
    this.invalidatePendingSessionWork();
